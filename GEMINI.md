...
- **Host 是默认交互主路径**：用户继续在已有 coding agent（Claude Code / Codex / Gemini CLI）中工作，Harnessly 寄生在宿主中自动介入。
...
在宿主主路径下，通过宿主的 **Sub-agent 机制** 落地：

| 角色 | 承担者 | 模型层级 |
|---|---|---|
| **Planner** | 宿主派生的 sub-agent（轻量模型：Haiku / gpt-5.4-mini / Gemini Flash） | 结构化填空，不需要最强推理 |
| **Generator** | 宿主主 Agent 自身（用户默认模型） | 代码生成需要最强推理能力 |
| **Evaluator** | 宿主派生的独立 sub-agent（中等模型：Sonnet / gpt-5.4 / Gemini Pro） | 需理解检查结果，但不需要代码生成级推理 |
...
- `cli` 只做参数解析 + 用户交互，不包含业务逻辑
- `core` 包含全部引擎逻辑，无 CLI 依赖，可独立测试
- `shared` 只放 Zod schema 和纯工具函数，被 cli 和 core 共同依赖
- `hosts/*` 只做宿主薄壳生成，不持有 task 状态或业务规则
...
2. **不要把宿主配置当 source-of-truth**：`.claude/`、`.codex/`、`.gemini/` 只是生成物。
3. **不要写入用户 home 目录**：`~/.claude`、`~/.codex`、`~/.gemini` 全部禁止。
4. **不要做执行期细粒度 runtime 控制**：`PreToolUse / PostToolUse` 只能是附加观测，不承载产品价值。
5. **不要把简单问题升级成系统性重构**：第一版做可落地的交付闭环，不做无限可编排平台。
6. **不要堆 runtime 技巧**：每增加一个 harness 机制，先问"它是在补产品层缺口，还是在补当前模型短板？"如果是后者，必须保持可删减。
7. **不要声称完成但未验证**：必须跑 `pnpm build && pnpm test && pnpm typecheck` 确认通过后才能声称完成。
