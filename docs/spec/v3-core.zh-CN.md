...
5 个角色 sub-agent（PM 是宿主主 agent，不是 sub-agent）必须通过宿主原生 sub-agent 机制实例化。**禁止**单 agent 切换角色：符合规范的实现必须保证角色间冷上下文隔离，否则：
...
对于无原生 sub-agent 的宿主：实现可降级到「lite」3 角色配置（requirement / developer / tester 合并），但必须向用户给出警告，说明角色分离仅是建议性的而非强制的。
