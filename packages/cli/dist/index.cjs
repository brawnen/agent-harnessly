...
var import_core20 = require("@harnessly/core");
...
async function appendHarnessEvent(workDir, event) {
  const harnessDir = (0, import_core.getHarnessPaths)(workDir).harnessDir;
  await (0, import_promises.mkdir)(harnessDir, { recursive: true });
  await (0, import_promises.appendFile)(
    import_node_path.default.join(harnessDir, "events.jsonl"),
    `${JSON.stringify({
...
async function runArchive(flags, positionals) {
  ...
  if (subcommand === "list") {
    const topics = await (0, import_core2.listArchiveTopics)(workDir);
    if (flags.json) {
      printJson(topics);
  ...
  if (subcommand === "show") {
    ...
    const detail = await (0, import_core2.showArchiveTopic)(workDir, topic2);
  ...
  if (subcommand === "verify") {
    const results = await (0, import_core2.verifyArchive)(workDir);
...
\u517C\u5BB9\u65E7\u8BED\u6CD5\uFF1Aharness archive requirement|design|both <task-id> [--topic] [--force]`
  ...
  const manager = new import_core2.TaskManager();
  ...
  const result = await (0, import_core2.archiveTaskArtifacts)(workDir, taskId, rawKind, { topic, force });
  await appendHarnessEvent(workDir, {
    ...
    files: result.files.map((f) => ({
      target: import_node_path2.default.relative(workDir, f.target),
      status: f.status
    }))
  });
  if (flags.json) {
    printJson({
      taskId: result.taskId,
      topic: result.topic,
      targetDir: import_node_path2.default.relative(workDir, result.targetDir),
      files: result.files.map((f) => ({
        source: f.source.startsWith(workDir) ? import_node_path2.default.relative(workDir, f.source) : f.source,
        target: import_node_path2.default.relative(workDir, f.target),
        status: f.status
      }))
    });
  ...
  printLines([
    `\u5F52\u6863\u5B8C\u6210\uFF1A${result.taskId} \u2192 ${import_node_path2.default.relative(workDir, result.targetDir)}`,
    `- topic: ${result.topic}`,
    ...result.files.map((f) => `- ${import_node_path2.default.relative(workDir, f.target)} [${f.status}]`)
  ]);
}
async function handlePromote(workDir, flags, positionals) {
  ...
  const mode = modeStr;
  const result = await (0, import_core2.promoteTaskArtifacts)(workDir, taskId, { topic, files, mode });
  await appendHarnessEvent(workDir, {
    ...
    mode,
    files: result.files.map((f) => ({
      source: import_node_path2.default.relative(workDir, f.source),
      target: import_node_path2.default.relative(workDir, f.target),
      status: f.status
    }))
  });
  ...
  printLines([
    `\u664B\u5347\u5B8C\u6210\uFF1A${result.taskId} \u2192 ${import_node_path2.default.relative(workDir, result.targetDir)}`,
    `- topic: ${result.topic}`,
    `- mode: ${mode}`,
    ...result.files.map((f) => `- ${import_node_path2.default.relative(workDir, f.target)} [${f.status}]`)
  ]);
...
async function runEvidenceBaseline(flags) {
  ...
  if (flags.show === true) {
    const baseline2 = await (0, import_core3.loadEvidenceBaseline)(workDir);
    if (!baseline2) {
      if (asJson) {
      ...
      printLines([
        ...
        `- \u8DEF\u5F84: ${(0, import_core3.getEvidenceBaselinePath)(workDir)}`,
  ...
  if (flags.clear === true) {
    try {
      await (0, import_promises2.unlink)((0, import_core3.getEvidenceBaselinePath)(workDir));
    } catch (error) {
  ...
  const config = await (0, import_core3.loadHarnessConfig)(workDir);
  const evidence = await (0, import_core3.collectEvidence)(workDir, config);
  const baseline = (0, import_core3.buildEvidenceBaseline)(evidence);
  await (0, import_core3.saveEvidenceBaseline)(workDir, baseline);
  await appendHarnessEvent(workDir, {
...
async function runFeedback(args) {
  ...
  if (subcommand === "list") {
    const entries = await (0, import_core4.loadFeedbackPool)(workDir);
    printLines(
      entries.map(
  ...
  if (subcommand === "promote") {
    ...
    const entry = await (0, import_core4.promoteFeedbackEntry)(workDir, taskId, reason);
...
var import_core6 = require("@harnessly/core");
...
async function installGitHooks(workDir, hosts) {
  const installed = [];
  const hooksDir = import_node_path3.default.join(workDir, ".git", "hooks");
  await (0, import_promises3.mkdir)(hooksDir, { recursive: true });
  const harnessBin = 'node "$(git rev-parse --show-toplevel)/packages/cli/dist/index.js"';
  const prePushPath = import_node_path3.default.join(hooksDir, "pre-push");
  const prePushScript = [
    "#!/bin/bash",
  ...
  await (0, import_promises3.chmod)(prePushPath, 493);
  installed.push(".git/hooks/pre-push");
  const preMergePath = import_node_path3.default.join(hooksDir, "pre-merge-commit");
  ...
  await (0, import_promises3.chmod)(preMergePath, 493);
...
async function copyFileOrWrite(filePath, content) {
  ...
  const existing = await (0, import_promises3.readFile)(filePath, "utf8").catch(() => null);
...
var import_core5 = require("@harnessly/core");
var import_host_claude_code = require("@harnessly/host-claude-code");
var import_host_codex = require("@harnessly/host-codex");
var import_host_shared = require("@harnessly/host-shared");
async function readFileIfExists(filePath) {
  try {
    return await (0, import_promises4.readFile)(filePath, "utf8");
  } catch {
    return null;
  }
}
...
async function ensureHostManifest(workDir, host) {
  const manifestPath = import_node_path4.default.join((0, import_core5.getHarnessPaths)(workDir).hostsDir, (0, import_host_shared.getHostManifestFilename)(host));
  const existing = await readFileIfExists(manifestPath);
  const commandPrefix = getCurrentHarnesslyCommand();
  if (existing) {
    const manifest2 = refreshManifestCommand((0, import_host_shared.parseHostManifest)(existing), commandPrefix);
    await (0, import_promises4.writeFile)(manifestPath, (0, import_host_shared.serializeHostManifest)(manifest2), "utf8");
    return manifest2;
  }
  const manifest = (0, import_host_shared.createHostManifest)(host, commandPrefix);
  await (0, import_promises4.writeFile)(manifestPath, (0, import_host_shared.serializeHostManifest)(manifest), "utf8");
  return manifest;
}
async function loadEnabledHosts(workDir, requestedHost) {
  ...
  const config = await (0, import_core5.loadHarnessConfig)(workDir);
...
function renderRepoLocalShell(manifest, config, agentManifests = []) {
  switch (manifest.host) {
    case "claude-code":
      return (0, import_host_claude_code.renderClaudeCodeManagedFiles)(manifest, { agentManifests });
    case "codex":
      return (0, import_host_codex.renderCodexManagedFiles)(manifest, {
...
async function installHostShells(workDir, requestedHost) {
  ...
  const config = await (0, import_core5.loadHarnessConfig)(workDir);
  const agentManifests = await (0, import_core5.loadAgentManifests)(workDir);
  for (const host of hosts) {
    ...
    for (const [relativePath, content] of Object.entries(files)) {
      const absolutePath = import_node_path4.default.join(workDir, relativePath);
      await (0, import_promises4.mkdir)(import_node_path4.default.dirname(absolutePath), { recursive: true });
      await (0, import_promises4.writeFile)(absolutePath, content, "utf8");
...
async function collectHostStatus(workDir) {
  ...
  for (const host of config.enabledHosts) {
    ...
    for (const [relativePath, expectedContent] of Object.entries(expectedFiles)) {
      const actual = await readFileIfExists(import_node_path4.default.join(workDir, relativePath));
      if (actual === null) {
        shellStatus = "missing";
        break;
      }
...
async function readActiveTaskId(workDir) {
  const filePath = (0, import_core5.getHarnessPaths)(workDir).activeTaskFile;
...
async function runInit(flags) {
  ...
  const force = flags.force === true;
  const projectType = await (0, import_core6.detectProjectType)(workDir);
  const paths = await (0, import_core6.ensureHarnessDirectories)(workDir);
  const config = (0, import_core6.createDefaultHarnessConfig)(projectType, hosts);
  const configStatus = await (0, import_core6.writeHarnessConfig)(workDir, config, force);
  const structureRulesStatus = await (0, import_core6.writeFileIfChanged)(
    paths.structureRulesFile,
    (0, import_core6.renderStructureRulesTemplate)(),
    force
  );
  const reviewAgentsStatus = await (0, import_core6.writeFileIfChanged)(
    paths.reviewAgentsFile,
    (0, import_core6.renderReviewAgentsTemplate)(),
    force
  );
  const skillResults = await (0, import_core6.writeDefaultSkillManifests)(
    workDir,
    projectType,
  ...
  const agentResults = await (0, import_core6.writeDefaultAgentManifests)(workDir, force);
  ...
  await (0, import_promises5.writeFile)(paths.activeTaskFile, "", "utf8");
  printLines([
    "Harnessly \u521D\u59CB\u5316\u5B8C\u6210",
...
var import_core7 = require("@harnessly/core");
function readStringFlag2(flags, name) {
  return typeof flags[name] === "string" ? flags[name].trim() : "";
}
function normalizeAgent(value) {
  const stripped = value.startsWith("harness-") ? value.slice("harness-".length) : value;
  if (import_core7.AGENT_ROLES.includes(stripped)) {
    return stripped;
  }
  throw new Error(
    `\u7F3A\u5C11\u6216\u975E\u6CD5 agent\u3002\u5141\u8BB8\u503C\uFF1A${import_core7.AGENT_ROLES.join(", ")}\uFF08\u4EA6\u63A5\u53D7 harness-<role> \u524D\u7F00\uFF09`
  );
}
...
var import_core8 = require("@harnessly/core");
...
async function runHostArtifactGuard(flags) {
  ...
  filePath = (0, import_node_path5.resolve)(cwd, filePath);
  const result = await (0, import_core8.checkWritePermission)(cwd, filePath, taskId);
...
var import_core9 = require("@harnessly/core");
...
async function loadReportCommitReady(filePath) {
  try {
    const report = JSON.parse(await (0, import_promises6.readFile)(filePath, "utf8"));
...
async function loadContractIfExists(filePath) {
  try {
    return (0, import_shared.parseContract)(await (0, import_promises6.readFile)(filePath, "utf8"));
  } catch {
    return null;
  }
}
...
async function countPreviousBlocks(workDir, taskId) {
  const eventsFile = import_node_path6.default.join((0, import_core9.getHarnessPaths)(workDir).harnessDir, "events.jsonl");
  try {
    const text = await (0, import_promises6.readFile)(eventsFile, "utf8");
    let count = 0;
...
async function resolveCompletionRecommendedAgent(workDir, activeTaskId) {
  const manifests = await (0, import_core9.loadAgentManifests)(workDir);
  const enabledRoles = (0, import_core9.collectEnabledRoles)(manifests);
  let stage = null;
  let failureStage = null;
  try {
    const ctx = await new import_core9.TaskManager().load(activeTaskId, workDir);
    stage = ctx.state.currentStage;
    failureStage = ctx.state.lastFailureStage ?? null;
  } catch {
  }
  const targetStage = failureStage ?? stage;
  const agent = (0, import_core9.pickRecommendedAgent)("completion_review", targetStage, enabledRoles);
...
async function runHostCompletionGate(flags, positionals) {
  ...
  const paths = (0, import_core9.getHarnessPaths)(workDir);
  const reportFile = import_node_path6.default.join(paths.tasksDir, activeTaskId, "report.json");
  const contractFile = import_node_path6.default.join(paths.tasksDir, activeTaskId, "contract.yaml");
  const evalCommand = `harnessly eval ${activeTaskId}`;
  const changedFiles = await (0, import_core9.collectChangedFiles)(workDir);
  const contract = await loadContractIfExists(contractFile);
  const scopeCheck = (0, import_core9.runScopeCheck)(contract ?? void 0, changedFiles);
...
var import_core10 = require("@harnessly/core");
...
async function runHostResidentReview(flags) {
  ...
  const result = await (0, import_core10.runResidentReview)(workDir, trigger, taskId);
  if (flags.json) {
    printJson(result);
    return;
  }
...
var import_core11 = require("@harnessly/core");
...
async function runHostSessionStart() {
  ...
  let status = null;
  if (activeTaskId) {
    const ctx = await new import_core11.TaskManager().load(activeTaskId, process.cwd());
...
var import_core13 = require("@harnessly/core");

// src/utils/planner-fallback.ts
var import_promises7 = require("fs/promises");
var import_node_path7 = __toESM(require("path"), 1);
var import_core12 = require("@harnessly/core");
function getPendingFile(workDir) {
  return import_node_path7.default.join((0, import_core12.getHarnessPaths)(workDir).harnessDir, "pending-planner-delegation.json");
}
async function readPendingDelegation(workDir) {
  try {
    return JSON.parse(await (0, import_promises7.readFile)(getPendingFile(workDir), "utf8"));
  } catch {
    return null;
  }
}
...
async function clearPendingDelegation(workDir) {
  try {
    await (0, import_promises7.unlink)(getPendingFile(workDir));
  } catch {
  }
}
...
async function resolveRecommendedAgent(workDir, action, activeStage) {
  ...
  const manifests = await (0, import_core13.loadAgentManifests)(workDir);
  const enabledRoles = (0, import_core13.collectEnabledRoles)(manifests);
  if (action === "delegate_to_planner") {
    return (0, import_core13.pickRecommendedAgent)("new_task", null, enabledRoles);
  }
  return (0, import_core13.pickRecommendedAgent)("resume_task", activeStage, enabledRoles);
}
async function runHostUserPromptSubmit(flags, positionals) {
  ...
  const manager = new import_core13.TaskManager();
  const config = await (0, import_core13.loadHarnessConfig)(workDir);
  const activeTaskId = await readActiveTaskId(workDir);
  ...
  if (action === "create_task") {
    ...
    const engine = new import_core13.WorkflowEngine(manager);
...
var import_core14 = require("@harnessly/core");
...
async function runEval(flags, positionals) {
  const workDir = process.cwd();
  const manager = new import_core14.TaskManager();
  ...
  const evidence = await (0, import_core14.collectEvidence)(workDir, ctx.config, ctx.contract);
  const adapter = previousReport?.adapter ?? createEvalOnlyAdapter();
  const commitGate = (0, import_core14.evaluateCommitGate)(evidence, adapter.exitCode);
  const report = (0, import_core14.createTaskReport)(ctx, adapter, evidence, commitGate);
...
var import_core15 = require("@harnessly/core");
async function runList(flags) {
  const manager = new import_core15.TaskManager();
  const workDir = process.cwd();
...
var import_core16 = require("@harnessly/core");
async function runRetry(flags, positionals) {
  const workDir = process.cwd();
  const manager = new import_core16.TaskManager();
  const requestedTaskId = (typeof flags["task-id"] === "string" ? flags["task-id"] : "") || (positionals[0] ?? "").trim();
  const taskId = requestedTaskId || await manager.getLatestTaskId(workDir);
  if (!taskId) {
  ...
  const engine = new import_core16.WorkflowEngine(manager);
...
// src/commands/run.ts
var import_core17 = require("@harnessly/core");
...
async function runTask(flags, positionals) {
  const workDir = process.cwd();
  const manager = new import_core17.TaskManager();
  ...
  const adapterKind = typeof flags.adapter === "string" ? flags.adapter : ctx.config.adapterKind;
  const adapterCommand = (typeof flags["adapter-command"] === "string" ? flags["adapter-command"] : ctx.config.adapterCommand) || "";
  const engine = new import_core17.WorkflowEngine(manager);
  await engine.run(ctx, {
    adapterKind,
    adapterCommand,
    dryRun: true
...
var import_core18 = require("@harnessly/core");
...
async function runStatus(flags, positionals) {
  const workDir = process.cwd();
  const manager = new import_core18.TaskManager();
...
var import_core19 = require("@harnessly/core");
async function runTemplatePromote(flags, positionals) {
  const workDir = process.cwd();
  const manager = new import_core19.TaskManager();
  const requestedTaskId = (typeof flags["task-id"] === "string" ? flags["task-id"] : "") || (positionals[0] ?? "").trim();
  const taskId = requestedTaskId || await manager.getLatestTaskId(workDir);
  if (!taskId) {
  ...
  const name = (typeof flags.name === "string" ? flags.name.trim() : "") || (0, import_core19.deriveTemplateName)(ctx.goal);
  const template = (0, import_core19.createTemplateDraft)(name, ctx.contract, report, ctx.config);
  const filePath = await (0, import_core19.saveTemplateDraft)(workDir, template);
  printLines([
    "\u6A21\u677F\u63D0\u5347\u5B8C\u6210",
    `- task_id: ${taskId}`,
    `- name: ${template.name}`,
...
function getCliRuntimeSummary() {
  return {
    ...
    version: "0.0.0",
    core: (0, import_core20.getCorePackageInfo)()
  };
}
