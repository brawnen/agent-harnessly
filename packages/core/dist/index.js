...
async function runFileCheck(index, workDir, target) {
  const filePath = path2.join(workDir, target);
...
async function loadPackageScripts(workDir) {
  const packageFile = path3.join(workDir, "package.json");
  if (!await fileExists3(packageFile)) {
    return {};
  }
  const pkg = JSON.parse(await readFile2(packageFile, "utf8"));
  return pkg.scripts ?? {};
}
...
async function saveTemplateDraft(workDir, template) {
  const templatesDir = path4.join(workDir, ".harness", "templates");
  const filePath = path4.join(templatesDir, `${template.name}.yaml`);
...
function getHarnessPaths(workDir) {
  const harnessDir = path5.join(workDir, HARNESS_DIRNAME);
  return {
    harnessDir,
    domainsDir: path5.join(harnessDir, "domains"),
    hostsDir: path5.join(harnessDir, "hosts"),
    tasksDir: path5.join(harnessDir, "tasks"),
    templatesDir: path5.join(harnessDir, "templates"),
    configFile: path5.join(harnessDir, "harness.config.yaml"),
    globalRulesFile: path5.join(harnessDir, "GLOBAL_RULES.md"),
    activeTaskFile: path5.join(harnessDir, "active-task.txt")
  };
}
async function ensureHarnessDirectories(workDir) {
  const paths = getHarnessPaths(workDir);
  await mkdir2(paths.harnessDir, { recursive: true });
  await mkdir2(paths.domainsDir, { recursive: true });
  await mkdir2(paths.hostsDir, { recursive: true });
  await mkdir2(paths.tasksDir, { recursive: true });
  await mkdir2(paths.templatesDir, { recursive: true });
  return paths;
}
...
async function writeFileIfChanged(filePath, content, force = false) {
  try {
    const existing = await readFile3(filePath, "utf8");
    if (existing === content) {
      return "skipped";
    }
    if (!force) {
...
async function loadHarnessConfig(workDir) {
  const configText = await readFile3(getHarnessPaths(workDir).configFile, "utf8");
...
import { mkdir as mkdir3, readdir, readFile as readFile4, writeFile as writeFile3 } from "fs/promises";
...
var TaskManager = class {
  ...
  getTasksDir(workDir) {
    return path6.join(workDir, ".harness", "tasks");
  }
  getActiveTaskFile(workDir) {
    return path6.join(workDir, ".harness", "active-task.txt");
  }
  getTaskDir(workDir, taskId) {
    return path6.join(this.getTasksDir(workDir), taskId);
  }
  getStateFile(taskDir) {
    return path6.join(taskDir, "state.json");
  }
  getMetaFile(taskDir) {
    return path6.join(taskDir, "task.json");
  }
  getContractFile(taskDir) {
    return path6.join(taskDir, "contract.yaml");
  }
  getPlanFile(taskDir) {
    return path6.join(taskDir, "plan.md");
  }
  getPromptFile(taskDir) {
    return path6.join(taskDir, "prompt.md");
  }
  getReportFile(taskDir) {
    return path6.join(taskDir, "report.json");
  }
  getFeedbackFile(taskDir) {
    return path6.join(taskDir, "feedback.md");
  }
  generateTaskId() {
  ...
  async listTasks(workDir) {
    ...
    const summaries = await Promise.all(
      entries.filter((entry) => entry.isDirectory()).map(async (entry) => {
        const taskDir = path6.join(tasksDir, entry.name);
