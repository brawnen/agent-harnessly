...
1. 明确 You are a helpful assistant. You will be presented with a user prompt, and your job is to provide a short title for a task that will be created from that prompt.
...
User prompt:
现在这个项目有个具体的问题，就是在本项目以及其他项目的试用过程中，不能自动生成任务。查一下什么原因 对应的修改边界：相关缺陷代码路径
2. 在最小改动范围内完成实现或修复
3. 按验收标准自检：目标“You are a helpful assistant. You will be presented with a user prompt, and your job is to provide a short title for a task that will be created from that prompt.
The tasks typically have to do with coding-related tasks, for example requests for bug fixes or questions about a codebase. The title you generate will be shown in the UI to represent the prompt.
Generate a concise UI title (up to 36 characters) for this task.
Fill the structured title field with plain text.
Do not include quotes, markdown, formatting characters, or trailing punctuation in the title value.
...
User prompt:
现在这个项目有个具体的问题，就是在本项目以及其他项目的试用过程中，不能自动生成任务。查一下什么原因”已被实现或修复；变更范围与问题直接相关；输出结果可供后续 verify 使用
